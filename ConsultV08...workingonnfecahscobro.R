...
server <- function(input, output, session) {
  ...
  # Almacena la información de los pacientes modificables
  pacientes_modificables_data <- reactiveVal(data.frame())
  
  # Observador para el campo de fecha del cobro
  observe({
    # Obtener la fecha seleccionada
    fecha_cobro <- input$diaCobro
    
    # Crear un vector de fechas para los próximos 12 meses
    fechas_siguientes <- seq(fecha_cobro, by = "months", length.out = 12)
    
    # Crear un vector de opciones de fecha en el formato deseado
    opciones_fecha <- sprintf("%s - %s", format(fechas_siguientes, "%d de %b"), format(fechas_siguientes, "%m/%d/%Y"))
    
    # Actualizar las opciones del campo de fecha del cobro
    updateDateInput(session, "diaCobro", value = fecha_cobro, label = paste("DÍA DEL COBRO\n", opciones_fecha, collapse = "\n"))
  })
  ...
  # Observador para el botón de modificar pacientes
  observeEvent(input$modificarPaciente, {
    # Obtener el paciente seleccionado
    paciente_seleccionado <- input$nombrePaciente
    
    # Filtrar los datos de pacientes para el paciente seleccionado
    paciente_modificable <- pacientes_data() %>%
      filter(NOMBRE == paciente_seleccionado)
    
    # Almacenar los datos del paciente seleccionado en pacientes_modificables_data
    pacientes_modificables_data(paciente_modificable)
  })
  ...
  # Actualiza la tabla de pacientes modificables
  output$pacientesModificablesTable <- renderTable({
    pacientes_modificables_data()
  })
}
