  ...
  theme = shinytheme("flatly"),
    tabsetPanel(type = "tabs",
      ...
      tabPanel("Agregado mensual",
               br(),
               # Plotly scatter plot
               plotlyOutput("scatter_plot_aggregated"),
               br(),
               # Data table
...
server <- function(input, output) {
  ...
  output$scatter_plot_aggregated <- renderPlotly({
    # Your plot_ly code here, using aggregated data (e.g., db.agg)
    plot_ly(db.agg, x = ~FECHA, y = ~TOTAL, color = ~MONEDA, type = 'scatter', mode = 'lines+markers') %>%
      layout(title = "Aggregated Monthly Data",
             xaxis = list(title = "Date"),
             yaxis = list(title = "Total Consultation Value"),
             showlegend = TRUE)
  })
